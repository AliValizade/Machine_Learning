...
class Perceptron:
    ...
    def predict(self, X_test):
        ...
        for x_test in X_test:
            y_pred = np.array(x_test) @ self.w + self.b
            y_pred = self.activation(y_pred)
...
class Perceptron_weather:
    ...
    def calculate_loss(self, X_test, Y_test, metric, batch_size=32):
        total_loss = 0
        ...
        for i in range(num_batches):
            ...
            if metric == 'mae':
                ...
                total_loss += np.sqrt(np.mean(error ** 2))
